...
export function update(){
  ...
  for(let i=game.enemies.length-1;i>=0;i--){ const e=game.enemies[i];
    ...
    e.fireCd--; if(e.fireCd<=0 && e.y>0){
      ...
      } else if(e.spinner){                                 // spinner: sling tangential horizontal bolts as it spins (#79)
    ...
    if(h.tele>0){ h.tele--;
      ...
      if(h.tele===0){                                   // warning ends → the shot lands
        if(h.kind==='circle'){ sfx.zoneBoom(); addShake(11);                    // ground detonation: boom + jolt…
          burst(h.x,h.y,h.hue,20,6.5,1,(h.radius||0)*0.75);                    // …a bit of debris kicked up across the blast disc…
          burst(h.x,h.y,32,6,2.5,1,(h.radius||0)*0.5);                          // …and a few warm embers scattered on top…
