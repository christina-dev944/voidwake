    ...
    if(h.tele>0){ h.tele--;
      ...
      if(h.tele===0){                                   // warning ends → the shot lands
        if(h.kind==='circle'){ sfx.zoneBoom(); addShake(11);                    // ground detonation: boom + jolt…
          burst(h.x,h.y,h.hue,40,6.5,1,(h.radius||0)*0.75);                    // …debris kicked up across the whole blast disc…
          burst(h.x,h.y,32,12,2.5,1,(h.radius||0)*0.5);                         // …a few warm embers scattered on top…
