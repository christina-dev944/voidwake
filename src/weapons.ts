...
export function enemyShoot(e: Enemy, pattern: string=e.pattern, spdMul=1, hue: number=e.hue) {
  ...
  switch(pattern) {
    ...
    case 'crescent': {   // breaker (#80): one curved arc aimed at the player. arcR = curvature radius, arcHalf = half-span (π/6 → 60° crescent)
      game.eBullets.push({ x:e.x, y:e.y, vx:Math.cos(aim)*spd, vy:Math.sin(aim)*spd, r:br, hue, shape, arcR:66, arcHalf:Math.PI/6 });
      break; }
  }
}
