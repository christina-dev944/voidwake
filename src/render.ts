...
export function draw(){
  ...
  for(const h of game.hazards){ if(h.kind!=='circle') continue; const R=h.radius||0;
    if(h.tele>0){
      ...
      ctx.globalAlpha=0.55*grow*grow*grow; ctx.strokeStyle='hsl(42,100%,85%)'; ctx.lineWidth=1;  // starts at 0, exponential fade-in (near-invisible until it's close, #61)
  ...
  // Lancer Skylance (#60): a brightening, pulsing charge line straight up during the
  ...
  if(game.state==='playing' && game.player){
    ...
    ctx.textAlign='left';
    drawBossBar();
    // MANUAL aim (#11): the aim point is shown by the OS crosshair cursor (set in main.ts) —
